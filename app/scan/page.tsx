      ...
      <div className="max-w-4xl mx-auto w-full px-4 py-10">
        ...
        <div className="bg-gray-900 border border-gray-800 rounded-2xl p-8 mb-6">
          <div className="border-2 border-dashed border-gray-700 rounded-xl p-8 text-center mb-6">
            ...
            <input type="file" accept=".pdf" onChange={(e) => setFile(e.target.files?.[0] || null)} className="hidden" id="fileInput" />
            <label htmlFor="fileInput" className="cursor-pointer bg-gray-800 hover:bg-gray-700 text-white px-6 py-3 rounded-xl text-sm font-medium transition-colors">Choose PDF File</label>
          ...
          <button onClick={handleUpload} disabled={!file || loading} className="w-full bg-blue-600 hover:bg-blue-700 disabled:opacity-50 disabled:cursor-not-allowed text-white py-3 rounded-xl font-medium transition-colors">
            {loading ? "Analyzing document..." : "Scan Document"}
          </button>
        </div>
        {error && <div className="bg-red-950 border border-red-800 rounded-xl p-4 mb-6 text-red-300 text-sm">⚠️ {error}</div>}
        {piiRedacted.length > 0 && <div className="bg-green-950 border border-green-800 rounded-xl p-4 mb-6 text-green-300 text-sm">🔒 <strong>Auto-redacted:</strong> {piiRedacted.join(", ")} detected and removed</div>}
        {result && (
